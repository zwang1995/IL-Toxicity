...
def Modeling(version, first_model, n1, n2, EPOCH):
    # Create floder
    floder_path = "./2_Output/FNN_" + str(version) + "/"
    floder_path_model = floder_path + "model/"

    create_floder(floder_path)
    create_floder(floder_path_model)

    # Load data
    StringPath = open("./1_Data/IPC81.csv", "r")
    ...
    if first_model == "FNN":
        X_train, X_test, Y_train, Y_test = train_test_split(items, [item.nor_prop for item in items], test_size=0.2)
    elif first_model == "SVM":
    ...
    # Create record file
    id = open(floder_path_model + "Training_record.csv", "w", newline="")
    ...
    for i in range(EPOCH):
        ...
        if (i + 1) % 1 == 0:
            ...
            id = open(floder_path_model + "Training_record.csv", "a", newline="")
            writer = csv.writer(id)
            writer.writerow([n1, n2, i + 1, RMSE_1, AAE_1, R2_1, RMSE_2, AAE_2, R2_2])
            id.close()

            if early_stopping.update:
                torch.save(model, floder_path_model + "FNN_model.pkl")

                ff = open(floder_path_model + "FNN_results.csv", "w", newline="")
...
if __name__ == "__main__":
    # This FNN-kFCV-Further.py file is used to train FNN models based on hyper-parameters obtained by FNN.kFCV.py file.
    version = "v1"
    first_model = "FNN"     # To ensure that FNN and SVM models share the same training and test sets.
                            # "FNN" means that the FNN model is trained first, and then the SVM model is trained.
                            # "SVM" means that the SVM model is trained first, and then the FNN model is trained.
    # n1 and n2 are determined by FNN.kFCV.py file.
    n1 = 16  # number of neurons in hidden layer 1
    n2 = 1  # number of neurons in hidden layer 1
    EPOCH = 100000  # define a large enough epoch number
    Modeling(version, first_model, n1, n2, EPOCH)
