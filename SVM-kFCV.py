...
def modeling(version, first_model):
    # Create floder
    floder_path = "./2_Output/SVM_" + str(version) + "/"
    floder_path_cv = floder_path + "cv/"
    floder_path_model = floder_path + "model/"

    create_floder(floder_path)
    create_floder(floder_path_cv)
    ...
    if first_model == "SVM":
        X_train, X_test, Y_train, Y_test = train_test_split(items, [item.property for item in items], test_size=0.2)
    elif first_model == "FNN":
    ...
    # Create record file
    id = open(floder_path_cv + "Cross_validation_record.csv", "w", newline="")
    ...
    for ci in c:
        for epsi in eps:
            ...
            id = open(floder_path_cv + "Cross_validation_record.csv", "a", newline="")
            writer = csv.writer(id)
            ...
            if np.average(scores) <= initial:
                initial = cv_score
                joblib.dump(model, floder_path_model + "SVM_model.pkl")

                ff = open(floder_path_model + "SVM_Results.csv", "w", newline="")
...
if __name__ == "__main__":
    # This SVM-kFCV.py file is used to obtain the optimal SVM model by synchronous cross validation and model training.
    version = "v1"
